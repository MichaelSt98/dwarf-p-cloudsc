...
MODULE CLOUDSC_GPU_SCC_CUF_DEVICE_ALLOCATE_MOD
  !USE NLEV_MOD, ONLY : NLEV

CONTAINS
ATTRIBUTES(GLOBAL) SUBROUTINE CLOUDSC_SCC_CUF_DEVICE_ALLOCATE (NLEV, KIDIA, KFDIA, KLON, KGPBLKS, PTSPHY, PT, PQ,  & !! 8
  ...
  & PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP )
    ...
    REAL(KIND=JPRB) :: ZDA
    REAL(KIND=JPRB), ALLOCATABLE :: ZLI(:) !device,
    !REAL(KIND=JPRB), device :: ZA(NLEV)
    ...
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZLIQFRAC(:)    ! cloud liquid water fraction: ql/(ql+qi)
    !REAL(KIND=JPRB), device :: ZICEFRAC(NLEV)    ! cloud ice water fraction: qi/(ql+qi)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZICEFRAC(:)    ! cloud ice water fraction: qi/(ql+qi)
    !REAL(KIND=JPRB), device:: ZQX(NLEV, NCLV)    ! water variables
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZQX(:, :)    ! water variables
    !REAL(KIND=JPRB), device:: ZQX0(NLEV, NCLV)    ! water variables at start of scheme
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZQX0(:, :)    ! water variables at start of scheme
    ...
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZPFPLSX(:,:)    ! generalized precipitation flux
    !REAL(KIND=JPRB), device :: ZLNEG(NLEV, NCLV)    ! for negative correction diagnostics
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZLNEG(:,:)    ! for negative correction diagnostics
    ...
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZQSLIQ(:)    ! liquid water saturation
...
#include "fccld.func.h"
      ...
      allocate(ZFOEALFA(NLEV + 1))
      allocate(ZTP1(NLEV))
      allocate(ZLI(NLEV)) !cudaMalloc(ZLI, KLEV) !
      allocate(ZA(NLEV))
      allocate(ZAORIG(NLEV))
      allocate(ZLIQFRAC(NLEV))
      allocate(ZICEFRAC(NLEV))
      allocate(ZQX(NLEV, NCLV))
      allocate(ZQX0(NLEV, NCLV))
      allocate(ZPFPLSX(NLEV + 1, NCLV))
      allocate(ZLNEG(NLEV, NCLV))
      allocate(ZPFPLSX(NLEV + 1, NCLV))
      allocate(ZLNEG(NLEV, NCLV))
      allocate(ZQXN2D(NLEV, NCLV))
      allocate(ZQSMIX(NLEV))
      allocate(ZQSLIQ(NLEV))    ! liquid water saturation
      allocate(ZQSICE(NLEV))
      allocate(ZFOEEWMT(NLEV))
      allocate(ZFOEEW(NLEV))
      allocate(ZFOEELIQT(NLEV))

      !cudaMalloc(ZFOEALFA, NLEV + 1)
      !cudaMalloc(ZTP1, NLEV)
      !cudaMalloc(ZLI, NLEV)
      !cudaMalloc(ZA, NLEV)
      !cudaMalloc(ZAORIG, NLEV)
      !cudaMalloc(ZLIQFRAC, NLEV)
      !cudaMalloc(ZICEFRAC, NLEV)
      !cudaMalloc(ZQX(NLEV, NCLV))
      !cudaMalloc(ZQX0(NLEV, NCLV))
      !cudaMalloc(ZPFPLSX(NLEV + 1, NCLV))
      !cudaMalloc(ZLNEG(NLEV, NCLV))
      !cudaMalloc(ZPFPLSX(NLEV + 1, NCLV))
      !cudaMalloc(ZLNEG(NLEV, NCLV))
      !cudaMalloc(ZQXN2D(NLEV, NCLV))
      !cudaMalloc(ZQSMIX(NLEV))
      !cudaMalloc(ZQSLIQ(NLEV))    ! liquid water saturation
      !cudaMalloc(ZQSICE(NLEV))
      !cudaMalloc(ZFOEEWMT(NLEV))
      !cudaMalloc(ZFOEEW(NLEV))
      !cudaMalloc(ZFOEELIQT(NLEV))
...
!!    END DO
      ...
      deallocate(ZQXN2D)
      deallocate(ZQSMIX)
      deallocate(ZQSLIQ)    ! liquid water saturation
      deallocate(ZQSICE)
      deallocate(ZFOEEWMT)
      deallocate(ZFOEEW)
      deallocate(ZFOEELIQT)
      ...
      !cudaFree(ZQXN2D)
      !cudaFree(ZQSMIX)
      !cudaFree(ZQSLIQ)    ! liquid water saturation
      !cudaFree(ZQSICE)
      !cudaFree(ZFOEEWMT)
      !cudaFree(ZFOEEW)
      !cudaFree(ZFOEELIQT)
...
END SUBROUTINE CLOUDSC_SCC_CUF_DEVICE_ALLOCATE
END MODULE CLOUDSC_GPU_SCC_CUF_DEVICE_ALLOCATE_MOD
