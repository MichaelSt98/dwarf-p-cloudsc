...
MODULE CLOUDSC_DRIVER_GPU_SCC_CUF_HOST_ALLOCATE_MOD
  ...
  USE CLOUDSC_GPU_SCC_CUF_HOST_ALLOCATE_MOD, ONLY: CLOUDSC_SCC_CUF_HOST_ALLOCATE
...
CONTAINS
  ...
  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_CUF_HOST_ALLOCATE( &
    ...
    INTEGER(KIND=JPIM)                                    :: KLEV, NUMOMP, NPROMA, NGPTOT, NGPBLKS, NGPTOTG
...
!!  device variables
    ...
    ! NEW
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZFOEALFA(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZTP1(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZLI(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZA(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZAORIG(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZLIQFRAC(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZICEFRAC(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE:: ZQX(:,:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE:: ZQX0(:,:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZPFPLSX(:,:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZLNEG(:,:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZQXN2D(:,:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZQSMIX(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZQSLIQ(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZQSICE(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZFOEEWMT(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZFOEEW(:,:,:)
    REAL(KIND=JPRB), device, ALLOCATABLE :: ZFOEELIQT(:,:,:)
    ! NEW
    ...
    !
    ! !NEW
    ! ALLOCATE(ZFOEALFA(NPROMA, NLEV + 1, NGPBLKS))
    ! ALLOCATE(ZTP1(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZLI(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZA(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZAORIG(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZLIQFRAC(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZICEFRAC(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZQX(NPROMA, NLEV, NCLV, NGPBLKS))
    ! ALLOCATE(ZQX0(NPROMA, NLEV, NCLV, NGPBLKS))
    ! ALLOCATE(ZPFPLSX(NPROMA, NLEV + 1, NCLV, NGPBLKS))
    ! ALLOCATE(ZLNEG(NPROMA, NLEV, NCLV, NGPBLKS))
    ! ALLOCATE(ZQXN2D(NPROMA, NLEV, NCLV, NGPBLKS))
    ! ALLOCATE(ZQSMIX(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZQSLIQ(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZQSICE(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZFOEEWMT(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZFOEEW(NPROMA, NLEV, NGPBLKS))
    ! ALLOCATE(ZFOEELIQT(NPROMA, NLEV, NGPBLKS))
    ! !NEW

    ALLOCATE( PT_d(NPROMA, KLEV, NGPBLKS),PQ_d(NPROMA, KLEV, NGPBLKS),BUFFER_CML_d(NPROMA,KLEV,3+NCLV,NGPBLKS))
    ALLOCATE( BUFFER_TMP_d(NPROMA,KLEV,3+NCLV,NGPBLKS), BUFFER_LOC_d(NPROMA,KLEV,3+NCLV,NGPBLKS))
    ALLOCATE( PVFA_d(NPROMA, KLEV, NGPBLKS), PVFL_d(NPROMA, KLEV, NGPBLKS), PVFI_d(NPROMA, KLEV, NGPBLKS) )
    ALLOCATE( PDYNA_d(NPROMA, KLEV, NGPBLKS), PDYNL_d(NPROMA, KLEV, NGPBLKS), PDYNI_d(NPROMA, KLEV, NGPBLKS) )
    ALLOCATE( PHRSW_d(NPROMA, KLEV, NGPBLKS), PHRLW_d(NPROMA, KLEV, NGPBLKS), PVERVEL_d(NPROMA, KLEV, NGPBLKS) )
    ALLOCATE( PAP_d(NPROMA, KLEV, NGPBLKS), PAPH_d(NPROMA, KLEV+1, NGPBLKS), PLSM_d(NPROMA, NGPBLKS) )
    ALLOCATE( LDCUM_d(NPROMA, NGPBLKS), KTYPE_d(NPROMA, NGPBLKS), PLU_d(NPROMA, KLEV, NGPBLKS) )
    ALLOCATE( PLUDE_d(NPROMA, KLEV, NGPBLKS), PSNDE_d(NPROMA, KLEV, NGPBLKS), PMFU_d(NPROMA, KLEV, NGPBLKS) )
    ALLOCATE( PMFD_d(NPROMA, KLEV, NGPBLKS), PA_d(NPROMA, KLEV, NGPBLKS), PCLV_d(NPROMA, KLEV, NCLV, NGPBLKS) )
    ALLOCATE( PSUPSAT_d(NPROMA, KLEV, NGPBLKS), PLCRIT_AER_d(NPROMA, KLEV, NGPBLKS), PICRIT_AER_d(NPROMA, KLEV, NGPBLKS) )
    ALLOCATE( PRE_ICE_d(NPROMA, KLEV, NGPBLKS), PCCN_d(NPROMA, KLEV, NGPBLKS),  PNICE_d(NPROMA, KLEV, NGPBLKS) )

    ALLOCATE(PCOVPTOT_d(NPROMA, KLEV, NGPBLKS), PRAINFRAC_TOPRFZ_d(NPROMA, NGPBLKS) )
    ALLOCATE(PFSQLF_d(NPROMA, KLEV+1, NGPBLKS), PFSQIF_d(NPROMA, KLEV+1, NGPBLKS), PFCQLNG_d(NPROMA, KLEV+1, NGPBLKS) )
    ALLOCATE(PFCQNNG_d(NPROMA, KLEV+1, NGPBLKS), PFSQRF_d(NPROMA, KLEV+1, NGPBLKS), PFSQSF_d(NPROMA, KLEV+1, NGPBLKS) )
    ALLOCATE(PFCQRNG_d(NPROMA, KLEV+1, NGPBLKS), PFCQSNG_d(NPROMA, KLEV+1, NGPBLKS) ,PFSQLTUR_d(NPROMA, KLEV+1, NGPBLKS) )
    ALLOCATE( PFSQITUR_d(NPROMA, KLEV+1, NGPBLKS))

    ALLOCATE(PFPLSL_d(NPROMA, KLEV+1, NGPBLKS), PFPLSN_d(NPROMA, KLEV+1, NGPBLKS))
    ALLOCATE(PFHPSL_d(NPROMA, KLEV+1, NGPBLKS), PFHPSN_d(NPROMA, KLEV+1, NGPBLKS) )

    !NEW
    ALLOCATE(ZFOEALFA(NPROMA, KLEV + 1, NGPBLKS))
    ALLOCATE(ZTP1(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZLI(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZA(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZAORIG(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZLIQFRAC(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZICEFRAC(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZQX(NPROMA, KLEV, NCLV, NGPBLKS))
    ALLOCATE(ZQX0(NPROMA, KLEV, NCLV, NGPBLKS))
    ALLOCATE(ZPFPLSX(NPROMA, KLEV + 1, NCLV, NGPBLKS))
    ALLOCATE(ZLNEG(NPROMA, KLEV, NCLV, NGPBLKS))
    ALLOCATE(ZQXN2D(NPROMA, KLEV, NCLV, NGPBLKS))
    ALLOCATE(ZQSMIX(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZQSLIQ(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZQSICE(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZFOEEWMT(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZFOEEW(NPROMA, KLEV, NGPBLKS))
    ALLOCATE(ZFOEELIQT(NPROMA, KLEV, NGPBLKS))
    !NEW
...
1003 format(5x,'NUMPROC=',i0,', NUMOMP=',i0,', NGPTOTG=',i0,', NPROMA=',i0,', NGPBLKS=',i0)
    ...
    CALL CLOUDSC_SCC_CUF_HOST_ALLOCATE<<<GRIDDIM,BLOCKDIM >>> &
        & (KLEV, ICSTART, ICEND, NPROMA, NGPBLKS, PTSPHY,&
        & PT_D,PQ_D, &
        & BUFFER_TMP_D, &
        & BUFFER_LOC_D, &
  ...
  END SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_CUF_HOST_ALLOCATE

END MODULE CLOUDSC_DRIVER_GPU_SCC_CUF_HOST_ALLOCATE_MOD
