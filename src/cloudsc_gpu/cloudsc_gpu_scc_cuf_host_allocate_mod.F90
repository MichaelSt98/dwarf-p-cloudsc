...
MODULE CLOUDSC_GPU_SCC_CUF_HOST_ALLOCATE_MOD
  !USE NLEV_MOD, ONLY : NLEV

CONTAINS
ATTRIBUTES(GLOBAL)  SUBROUTINE CLOUDSC_SCC_CUF_HOST_ALLOCATE (NLEV, KIDIA, KFDIA, KLON, KGPBLKS, PTSPHY, PT, PQ,  & !! 8
  ...
  & ZFOEALFA, ZTP1, ZLI, ZA, ZAORIG, ZLIQFRAC, ZICEFRAC, ZQX, &
  & ZQX0, ZPFPLSX, ZLNEG,  ZQXN2D, ZQSMIX, ZQSLIQ, ZQSICE, &
  & ZFOEEWMT, ZFOEEW, ZFOEELIQT)
    ...
    INTEGER(KIND=JPIM), INTENT(IN), VALUE :: NLEV    ! Number of grid points
    ...
    !! NEW
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEALFA(KLON, NLEV + 1,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT):: ZTP1(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZLI(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZA(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZAORIG(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZLIQFRAC(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZICEFRAC(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT):: ZQX(KLON, NLEV, NCLV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT):: ZQX0(KLON, NLEV, NCLV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZPFPLSX(KLON, NLEV + 1, NCLV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZLNEG(KLON, NLEV, NCLV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQXN2D(KLON, NLEV, NCLV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQSMIX(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQSLIQ(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZQSICE(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEEWMT(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEEW(KLON, NLEV,KGPBLKS)
    REAL(KIND=JPRB), INTENT(INOUT) :: ZFOEELIQT(KLON, NLEV,KGPBLKS)
    !! NEW
...
!! !$acc loop seq
      DO JK=1,NLEV
        ZTP1(JL,JK,JBLK) = PT(JL, JK, JBLK) + PTSPHY*TENDENCY_TMP(JL, JK, 1, JBLK)
        ZQX(JL,JK, NCLDQV,JBLK) = PQ(JL, JK, JBLK) + PTSPHY*TENDENCY_TMP(JL, JK, 3, JBLK)
        ZQX0(JL,JK, NCLDQV,JBLK) = PQ(JL, JK, JBLK) + PTSPHY*TENDENCY_TMP(JL, JK, 3, JBLK)
        ZA(JL,JK,JBLK) = PA(JL, JK, JBLK) + PTSPHY*TENDENCY_TMP(JL, JK, 2, JBLK)
        ZAORIG(JL,JK,JBLK) = PA(JL, JK, JBLK) + PTSPHY*TENDENCY_TMP(JL, JK, 2, JBLK)
      END DO
...
!! !$acc loop seq
      DO JM=1,NCLV - 1
        DO JK=1,NLEV
          ZQX(JL,JK, JM,JBLK) = PCLV(JL, JK, JM, JBLK) + PTSPHY*TENDENCY_TMP(JL, JK, 3+JM, JBLK)
          ZQX0(JL,JK, JM,JBLK) = PCLV(JL, JK, JM, JBLK) + PTSPHY*TENDENCY_TMP(JL, JK, 3+JM, JBLK)
        END DO
      END DO
...
!! !$acc loop seq
      DO JM=1,NCLV
        DO JK=1,NLEV + 1
          ZPFPLSX(JL,JK,JM,JBLK) = 0.0_JPRB            ! precip fluxes
        END DO
      END DO
...
!! !$acc loop seq
      DO JM=1,NCLV
        DO JK=1,NLEV
          ZQXN2D(JL,JK, JM,JBLK) = 0.0_JPRB            ! end of timestep values in 2D
          ZLNEG(JL,JK, JM,JBLK) = 0.0_JPRB            ! negative input check
        END DO
      END DO
...
!! !$acc loop seq
      DO JK=1,NLEV
!!        IF (ZQX(JL, JK, NCLDQL,JBLK) + ZQX(JL,JK, NCLDQI,JBLK) < YRECLDP%RLMIN .or. ZA(JL,JK,JBLK) < YRECLDP%RAMIN) THEN
        IF (ZQX(JL,JK, NCLDQL,JBLK) + ZQX(JL,JK, NCLDQI,JBLK) < YRECLDP%RLMIN .or. ZA(JL,JK,JBLK) < YRECLDP%RAMIN) THEN

          ! Evaporate small cloud liquid water amounts
          ZLNEG(JL,JK, NCLDQL,JBLK) = ZLNEG(JL,JK, NCLDQL,JBLK) + ZQX(JL, JK, NCLDQL,JBLK)
          ZQADJ = ZQX(JL,JK, NCLDQL,JBLK)*ZQTMST
          ...
          ZQX(JL,JK, NCLDQV,JBLK) = ZQX(JL,JK, NCLDQV,JBLK) + ZQX(JL, JK, NCLDQL,JBLK)
          ZQX(JL, JK, NCLDQL,JBLK) = 0.0_JPRB

          ! Evaporate small cloud ice water amounts
          ZLNEG(JL,JK, NCLDQI,JBLK) = ZLNEG(JL,JK, NCLDQI,JBLK) + ZQX(JL,JK, NCLDQI,JBLK)
          ZQADJ = ZQX(JL,JK, NCLDQI,JBLK)*ZQTMST
          TENDENCY_LOC(JL, JK, 3, JBLK) = TENDENCY_LOC(JL, JK, 3, JBLK) + ZQADJ
          TENDENCY_LOC(JL, JK, 1, JBLK) = TENDENCY_LOC(JL, JK, 1, JBLK) - RALSDCP*ZQADJ
          ZQX(JL,JK, NCLDQV,JBLK) = ZQX(JL,JK, NCLDQV,JBLK) + ZQX(JL,JK, NCLDQI,JBLK)
          ZQX(JL,JK, NCLDQI,JBLK) = 0.0_JPRB

          ! Set cloud cover to zero
          ZA(JL,JK,JBLK) = 0.0_JPRB
...
!! !$acc loop seq
      DO JM=1,NCLV - 1
        ...
        DO JK=1,NLEV
          !DIR$ IVDEP
!!          IF (ZQX(JL,JK, JM,JBLK) < YRECLDP%RLMIN) THEN
          IF (ZQX(JL,JK, JM,JBLK) < YRECLDP%RLMIN) THEN
            ZLNEG(JL,JK, JM,JBLK) = ZLNEG(JL,JK, JM,JBLK) + ZQX(JL,JK, JM,JBLK)
            ZQADJ = ZQX(JL,JK, JM,JBLK)*ZQTMST
            TENDENCY_LOC(JL, JK, 3, JBLK) = TENDENCY_LOC(JL, JK, 3, JBLK) + ZQADJ
            ...
            ZQX(JL,JK, NCLDQV,JBLK) = ZQX(JL,JK, NCLDQV,JBLK) + ZQX(JL,JK, JM,JBLK)
            ZQX(JL,JK, JM,JBLK) = 0.0_JPRB
          END IF
        END DO
      END DO
...
!! !$acc loop seq
      DO JK=1,NLEV
        ...
        !----------------------------------------
        ZFOEALFA(JL,JK,JBLK) = FOEALFA(ZTP1(JL,JK,JBLK))
        ZFOEEWMT(JL,JK,JBLK) = MIN(FOEEWM(ZTP1(JL,JK,JBLK)) / PAP(JL, JK, JBLK), 0.5_JPRB)
        ZQSMIX(JL,JK,JBLK) = ZFOEEWMT(JL,JK,JBLK)
        ZQSMIX(JL,JK,JBLK) = ZQSMIX(JL,JK,JBLK) / (1.0_JPRB - RETV*ZQSMIX(JL,JK,JBLK))

        !---------------------------------------------
        ! ice saturation T<273K
        ! liquid water saturation for T>273K
        !---------------------------------------------
        ZALFA = FOEDELTA(ZTP1(JL,JK,JBLK))
        ZFOEEW(JL,JK,JBLK) = MIN((ZALFA*FOEELIQ(ZTP1(JL,JK,JBLK)) + (1.0_JPRB - ZALFA)*FOEEICE(ZTP1(JL,JK,JBLK))) / PAP(JL, JK, JBLK), 0.5_JPRB)
        ZFOEEW(JL,JK,JBLK) = MIN(0.5_JPRB, ZFOEEW(JL,JK,JBLK))
        ZQSICE(JL,JK,JBLK) = ZFOEEW(JL,JK,JBLK) / (1.0_JPRB - RETV*ZFOEEW(JL,JK,JBLK))

        !----------------------------------
        ! liquid water saturation
        !----------------------------------
        ZFOEELIQT(JL,JK,JBLK) = MIN(FOEELIQ(ZTP1(JL,JK,JBLK)) / PAP(JL, JK, JBLK), 0.5_JPRB)
        ZQSLIQ(JL,JK,JBLK) = ZFOEELIQT(JL,JK,JBLK)
        ZQSLIQ(JL,JK,JBLK) = ZQSLIQ(JL,JK,JBLK) / (1.0_JPRB - RETV*ZQSLIQ(JL,JK,JBLK))

        !   !----------------------------------
        !   ! ice water saturation
        !   !----------------------------------
        !   ZFOEEICET(JL,JK, JBLK)=MIN(FOEEICE(ZTP1(JL,JK,JBLK))/PAP(JL,JK, JBLK),0.5_JPRB)
...
!! !$acc loop seq
      DO JK=1,NLEV
        ...
        !------------------------------------------
        ZA(JL,JK,JBLK) = MAX(0.0_JPRB, MIN(1.0_JPRB, ZA(JL,JK,JBLK)))

        !-------------------------------------------------------------------
        ! Calculate liq/ice fractions (no longer a diagnostic relationship)
        !-------------------------------------------------------------------
        ZLI(JL,JK,JBLK) = ZQX(JL, JK, NCLDQL,JBLK) + ZQX(JL,JK, NCLDQI,JBLK)
!!        IF (ZLI(JL,JK,JBLK) > YRECLDP%RLMIN) THEN
        IF (ZLI(JL,JK,JBLK) > YRECLDP%RLMIN) THEN
          ZLIQFRAC(JL,JK,JBLK) = ZQX(JL, JK, NCLDQL,JBLK) / ZLI(JL,JK,JBLK)
          ZICEFRAC(JL,JK,JBLK) = 1.0_JPRB - ZLIQFRAC(JL,JK,JBLK)
        ELSE
          ZLIQFRAC(JL,JK,JBLK) = 0.0_JPRB
          ZICEFRAC(JL,JK,JBLK) = 0.0_JPRB
        END IF
...
!! !$acc loop seq
      DO JK=1,NLEV - 1
        ...
        IF (ZSIG > 0.1_JPRB .and. ZSIG < 0.4_JPRB .and. ZTP1(JL,JK,JBLK) > ZTP1(JL,JK + 1,JBLK)) THEN
          ZTRPAUS = ZSIG
        END IF
      END DO
...
!!      DO JK=YRECLDP%NCLDTOP,NLEV
      DO JK=YRECLDP%NCLDTOP,NLEV
        ...
        ZGDP = RG / ZDP          ! g/dp
        ZRHO = PAP(JL, JK, JBLK) / ((RD*ZTP1(JL,JK,JBLK)))          ! p/RT air density
        ...
        ! liquid
        ZFACW = R5LES / ((ZTP1(JL,JK,JBLK) - R4LES)**2)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZFOEELIQT(JL,JK,JBLK))
        ZDQSLIQDT = ZFACW*ZCOR*ZQSLIQ(JL,JK,JBLK)
        ZCORQSLIQ = 1.0_JPRB + RALVDCP*ZDQSLIQDT

        ! ice
        ZFACI = R5IES / ((ZTP1(JL,JK,JBLK) - R4IES)**2)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZFOEEW(JL,JK,JBLK))
        ZDQSICEDT = ZFACI*ZCOR*ZQSICE(JL,JK,JBLK)
        ZCORQSICE = 1.0_JPRB + RALSDCP*ZDQSICEDT

        ! diagnostic mixed
        ZALFAW = ZFOEALFA(JL,JK,JBLK)
        ZALFAWM = ZALFAW
        ZFAC = ZALFAW*ZFACW + (1.0_JPRB - ZALFAW)*ZFACI
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZFOEEWMT(JL,JK,JBLK))
        ZDQSMIXDT = ZFAC*ZCOR*ZQSMIX(JL,JK,JBLK)
        ZCORQSMIX = 1.0_JPRB + FOELDCPM(ZTP1(JL,JK,JBLK))*ZDQSMIXDT

        ! evaporation/sublimation limits
        ZEVAPLIMMIX = MAX((ZQSMIX(JL,JK,JBLK) - ZQX(JL,JK, NCLDQV,JBLK)) / ZCORQSMIX, 0.0_JPRB)
        ZEVAPLIMLIQ = MAX((ZQSLIQ(JL,JK,JBLK) - ZQX(JL,JK, NCLDQV,JBLK)) / ZCORQSLIQ, 0.0_JPRB)
        ZEVAPLIMICE = MAX((ZQSICE(JL,JK,JBLK) - ZQX(JL,JK, NCLDQV,JBLK)) / ZCORQSICE, 0.0_JPRB)

        !--------------------------------
        ! in-cloud consensate amount
        !--------------------------------
        ZTMPA = 1.0_JPRB / MAX(ZA(JL,JK,JBLK), ZEPSEC)
        ZLIQCLD = ZQX(JL, JK, NCLDQL,JBLK)*ZTMPA
        ZICECLD = ZQX(JL,JK, NCLDQI,JBLK)*ZTMPA
...
!!        IF (ZQX(JL, JK, NCLDQL,JBLK) < YRECLDP%RLMIN) THEN
        IF (ZQX(JL, JK, NCLDQL,JBLK) < YRECLDP%RLMIN) THEN
          ZSOLQA(NCLDQV, NCLDQL) = ZQX(JL, JK, NCLDQL,JBLK)
          ZSOLQA(NCLDQL, NCLDQV) = -ZQX(JL, JK, NCLDQL,JBLK)
        END IF

!!        IF (ZQX(JL,JK, NCLDQI,JBLK) < YRECLDP%RLMIN) THEN
        IF (ZQX(JL,JK, NCLDQI,JBLK) < YRECLDP%RLMIN) THEN
          ZSOLQA(NCLDQV, NCLDQI) = ZQX(JL,JK, NCLDQI,JBLK)
          ZSOLQA(NCLDQI, NCLDQV) = -ZQX(JL,JK, NCLDQI,JBLK)
        END IF
        ...
        ZFOKOOP = FOKOOP(ZTP1(JL,JK,JBLK))

!!        IF (ZTP1(JL,JK,JBLK) >= RTT .or. YRECLDP%NSSOPT == 0) THEN
        IF (ZTP1(JL,JK,JBLK) >= RTT .or. YRECLDP%NSSOPT == 0) THEN
          ZFAC = 1.0_JPRB
          ZFACI = 1.0_JPRB
        ELSE
          ZFAC = ZA(JL,JK,JBLK) + ZFOKOOP*(1.0_JPRB - ZA(JL,JK,JBLK))
...
!!        IF (ZA(JL,JK,JBLK) > 1.0_JPRB - YRECLDP%RAMIN) THEN
        IF (ZA(JL,JK,JBLK) > 1.0_JPRB - YRECLDP%RAMIN) THEN
          ZSUPSAT = MAX((ZQX(JL,JK, NCLDQV,JBLK) - ZFAC*ZQSICE(JL,JK,JBLK)) / ZCORQSICE, 0.0_JPRB)
        ELSE
          ! Calculate environmental humidity supersaturation
          ZQP1ENV = (ZQX(JL,JK, NCLDQV,JBLK) - ZA(JL,JK,JBLK)*ZQSICE(JL,JK,JBLK)) / MAX(1.0_JPRB - ZA(JL,JK,JBLK), ZEPSILON)
          !& SIGN(MAX(ABS(1.0_JPRB-ZA(JL,JK, JBLK)),ZEPSILON),1.0_JPRB-ZA(JL,JK, JBLK))
          ZSUPSAT = MAX(((1.0_JPRB - ZA(JL,JK,JBLK))*(ZQP1ENV - ZFAC*ZQSICE(JL,JK,JBLK))) / ZCORQSICE, 0.0_JPRB)
        END IF
...
!!          IF (ZTP1(JL,JK,JBLK) > YRECLDP%RTHOMO) THEN
          IF (ZTP1(JL,JK,JBLK) > YRECLDP%RTHOMO) THEN
          ...
          ZSOLAC = (1.0_JPRB - ZA(JL,JK,JBLK))*ZFACI
...
!!        IF (JK > YRECLDP%NCLDTOP) THEN
        IF (JK > YRECLDP%NCLDTOP) THEN
          ...
          ZDTDP = (ZRDCP*0.5_JPRB*(ZTP1(JL,JK - 1,JBLK) + ZTP1(JL,JK,JBLK))) / PAPH(JL, JK, JBLK)
...
!!        IF (KTYPE(JL,JBLK) > 0 .and. PLUDE(JL, JK, JBLK) > ZEPSEC)         ZLDIFDT = YRECLDP%RCLDIFF_CONVI*ZLDIFDT
        ...
        IF (ZLI(JL,JK,JBLK) > ZEPSEC) THEN
          ...
          ZE = ZLDIFDT*MAX(ZQSMIX(JL,JK,JBLK) - ZQX(JL,JK, NCLDQV,JBLK), 0.0_JPRB)
          ZLEROS = ZA(JL,JK,JBLK)*ZE
          ZLEROS = MIN(ZLEROS, ZEVAPLIMMIX)
          ZLEROS = MIN(ZLEROS, ZLI(JL,JK,JBLK))
          ZAEROS = ZLEROS / ZLICLD            !if linear term

          ! Erosion is -ve LINEAR in L,A
          ZSOLAC = ZSOLAC - ZAEROS            !linear

          ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + ZLIQFRAC(JL,JK,JBLK)*ZLEROS
          ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - ZLIQFRAC(JL,JK,JBLK)*ZLEROS
          ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + ZICEFRAC(JL,JK,JBLK)*ZLEROS
          ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - ZICEFRAC(JL,JK,JBLK)*ZLEROS
        ...
        ZDTDP = (ZRDCP*ZTP1(JL,JK,JBLK)) / PAP(JL, JK, JBLK)
        ZDPMXDT = ZDP*ZQTMST
        ZMFDN = 0.0_JPRB
        ...
        ZQOLD = ZQSMIX(JL,JK,JBLK)
        ZTOLD = ZTP1(JL,JK,JBLK)
        ZTP1(JL,JK,JBLK) = ZTP1(JL,JK,JBLK) + ZDTFORC
        ZTP1(JL,JK,JBLK) = MAX(ZTP1(JL,JK,JBLK), 160.0_JPRB)
        LLFLAG = .true.

        ! Formerly a call to CUADJTQ(..., ICALL=5)
        ZQP = 1.0_JPRB / PAP(JL, JK, JBLK)
        ZQSAT = FOEEWM(ZTP1(JL,JK,JBLK))*ZQP
        ZQSAT = MIN(0.5_JPRB, ZQSAT)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
        ZQSAT = ZQSAT*ZCOR
        ZCOND = (ZQSMIX(JL,JK,JBLK) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(ZTP1(JL,JK,JBLK)))
        ZTP1(JL,JK,JBLK) = ZTP1(JL,JK,JBLK) + FOELDCPM(ZTP1(JL,JK,JBLK))*ZCOND
        ZQSMIX(JL,JK,JBLK) = ZQSMIX(JL,JK,JBLK) - ZCOND
        ZQSAT = FOEEWM(ZTP1(JL,JK,JBLK))*ZQP
        ZQSAT = MIN(0.5_JPRB, ZQSAT)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
        ZQSAT = ZQSAT*ZCOR
        ZCOND1 = (ZQSMIX(JL,JK,JBLK) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(ZTP1(JL,JK,JBLK)))
        ZTP1(JL,JK,JBLK) = ZTP1(JL,JK,JBLK) + FOELDCPM(ZTP1(JL,JK,JBLK))*ZCOND1
        ZQSMIX(JL,JK,JBLK) = ZQSMIX(JL,JK,JBLK) - ZCOND1

        ZDQS = ZQSMIX(JL,JK,JBLK) - ZQOLD
        ZQSMIX(JL,JK,JBLK) = ZQOLD
        ZTP1(JL,JK,JBLK) = ZTOLD
        ...
        IF (ZDQS > 0.0_JPRB) THEN
          ...
          ZLEVAP = ZA(JL,JK,JBLK)*MIN(ZDQS, ZLICLD)
          ZLEVAP = MIN(ZLEVAP, ZEVAPLIMMIX)
          ZLEVAP = MIN(ZLEVAP, MAX(ZQSMIX(JL,JK,JBLK) - ZQX(JL,JK, NCLDQV,JBLK), 0.0_JPRB))

          ! For first guess call
          ZLEVAPL = ZLIQFRAC(JL,JK,JBLK)*ZLEVAP
          ZLEVAPI = ZICEFRAC(JL,JK,JBLK)*ZLEVAP

          ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + ZLIQFRAC(JL,JK,JBLK)*ZLEVAP
          ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - ZLIQFRAC(JL,JK,JBLK)*ZLEVAP

          ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + ZICEFRAC(JL,JK,JBLK)*ZLEVAP
          ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - ZICEFRAC(JL,JK,JBLK)*ZLEVAP
...
!!        IF (ZA(JL,JK,JBLK) > ZEPSEC .and. ZDQS <= -YRECLDP%RLMIN) THEN
        IF (ZA(JL,JK,JBLK) > ZEPSEC .and. ZDQS <= -YRECLDP%RLMIN) THEN

          ZLCOND1 = MAX(-ZDQS, 0.0_JPRB)            !new limiter

          !old limiter (significantly improves upper tropospheric humidity rms)
          IF (ZA(JL,JK,JBLK) > 0.99_JPRB) THEN
            ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSMIX(JL,JK,JBLK))
            ZCDMAX = (ZQX(JL,JK, NCLDQV,JBLK) - ZQSMIX(JL,JK,JBLK)) / (1.0_JPRB + ZCOR*ZQSMIX(JL,JK,JBLK)*FOEDEM(ZTP1(JL,JK,JBLK)))
          ELSE
            ZCDMAX = (ZQX(JL,JK, NCLDQV,JBLK) - ZA(JL,JK,JBLK)*ZQSMIX(JL,JK,JBLK)) / ZA(JL,JK,JBLK)
          END IF
          ZLCOND1 = MAX(MIN(ZLCOND1, ZCDMAX), 0.0_JPRB)
          ! end old limiter

          ZLCOND1 = ZA(JL,JK,JBLK)*ZLCOND1
...
!!        IF (ZDQS <= -YRECLDP%RLMIN .and. ZA(JL,JK,JBLK) < 1.0_JPRB - ZEPSEC) THEN
        IF (ZDQS <= -YRECLDP%RLMIN .and. ZA(JL,JK,JBLK) < 1.0_JPRB - ZEPSEC) THEN
...
!!          IF (YRECLDP%NSSOPT == 0) THEN
          IF (YRECLDP%NSSOPT == 0) THEN
            ! No scheme
            ZQE = (ZQX(JL,JK, NCLDQV,JBLK) - ZA(JL,JK,JBLK)*ZQSICE(JL,JK,JBLK)) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL,JK,JBLK))
...
!!          ELSE IF (YRECLDP%NSSOPT == 2) THEN
          ELSE IF (YRECLDP%NSSOPT == 2) THEN
            ! Lohmann and Karcher
            ZQE = ZQX(JL,JK, NCLDQV,JBLK)
!!          ELSE IF (YRECLDP%NSSOPT == 3) THEN
          ELSE IF (YRECLDP%NSSOPT == 3) THEN
            ! Gierens
            ZQE = ZQX(JL,JK, NCLDQV,JBLK) + ZLI(JL,JK,JBLK)
          END IF
...
!!          IF (ZTP1(JL,JK,JBLK) >= RTT .or. YRECLDP%NSSOPT == 0) THEN
          ...
          IF (ZQE >= ZRHC*ZQSICE(JL,JK,JBLK)*ZFAC .and. ZQE < ZQSICE(JL,JK,JBLK)*ZFAC) THEN
            ! note: not **2 on 1-a term if ZQE is used.
            ! Added correction term ZFAC to numerator 15/03/2010
            ZACOND = -((1.0_JPRB - ZA(JL,JK,JBLK))*ZFAC*ZDQS) / MAX(2.0_JPRB*(ZFAC*ZQSICE(JL,JK,JBLK) - ZQE), ZEPSEC)

            ZACOND = MIN(ZACOND, 1.0_JPRB - ZA(JL,JK,JBLK))              !PUT THE LIMITER BACK
            ...
            ZZDL = (2.0_JPRB*(ZFAC*ZQSICE(JL,JK,JBLK) - ZQE)) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL,JK,JBLK))
            ! Added correction term ZFAC 15/03/2010
            IF (ZFAC*ZDQS < -ZZDL) THEN
              ! ZLCONDLIM=(ZA(JL,JK, JBLK)-1.0_JPRB)*ZDQS(JK)-ZQSICE(JL,JK, JBLK)+ZQX(JL,JK,NCLDQV)
              ZLCONDLIM = (ZA(JL,JK,JBLK) - 1.0_JPRB)*ZFAC*ZDQS - ZFAC*ZQSICE(JL,JK,JBLK) + ZQX(JL,JK, NCLDQV,JBLK)
              ZLCOND2 = MIN(ZLCOND2, ZLCONDLIM)
            END IF
            ZLCOND2 = MAX(ZLCOND2, 0.0_JPRB)

!!            IF (ZLCOND2 < YRECLDP%RLMIN .or. (1.0_JPRB - ZA(JL,JK,JBLK)) < ZEPSEC) THEN
            IF (ZLCOND2 < YRECLDP%RLMIN .or. (1.0_JPRB - ZA(JL,JK,JBLK)) < ZEPSEC) THEN
              ZLCOND2 = 0.0_JPRB
              ZACOND = 0.0_JPRB
            END IF
...
!!          IF (ZA(JK - 1) < YRECLDP%RCLDTOPCF .and. ZA(JL,JK,JBLK) >= YRECLDP%RCLDTOPCF) THEN
          IF (ZA(JL, JK - 1,JBLK) < YRECLDP%RCLDTOPCF .and. ZA(JL,JK,JBLK) >= YRECLDP%RCLDTOPCF) THEN
            ZCLDTOPDIST = 0.0_JPRB
          ELSE
          ...
          !--------------------------------------------------------------
!!          IF (ZTP1(JL,JK,JBLK) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
          IF (ZTP1(JL,JK,JBLK) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
            ! T<273K

            ZVPICE = (FOEEICE(ZTP1(JL,JK,JBLK))*RV) / RD
            ...
            !------------------------------------------------
            ZADD = (RLSTT*(RLSTT / ((RV*ZTP1(JL,JK,JBLK))) - 1.0_JPRB)) / ((2.4E-2_JPRB*ZTP1(JL,JK,JBLK)))
            ZBDD = (RV*ZTP1(JL,JK,JBLK)*PAP(JL, JK, JBLK)) / ((2.21_JPRB*ZVPICE))
...
!!            ZICE0 = MAX(ZICECLD, (ZICENUCLEI*YRECLDP%RICEINIT) / ZRHO)
            ...
            !---------------------------
            ZDEPOS = MAX(ZA(JL,JK,JBLK)*(ZINEW - ZICE0), 0.0_JPRB)
...
!!            ZICE0 = MAX(ZICECLD, (ZICENUCLEI*YRECLDP%RICEINIT) / ZRHO)
            ...
            ZAPLUSB =  &
!!            & YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL,JK,JBLK) + PAP(JL, JK, JBLK)*YRECLDP%RCL_APB3*ZTP1(JL,JK,JBLK)**3._JPRB
            & YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL,JK,JBLK) + PAP(JL, JK, JBLK)*YRECLDP%RCL_APB3*ZTP1(JL,JK,JBLK)**3._JPRB
            ZCORRFAC = (1.0_JPRB / ZRHO)**0.5_JPRB
            ZCORRFAC2 = ((ZTP1(JL,JK,JBLK) / 273.0_JPRB)**1.5_JPRB)*(393.0_JPRB / (ZTP1(JL,JK,JBLK) + 120.0_JPRB))

!!            ZPR02 = (ZRHO*ZICE0*YRECLDP%RCL_CONST1I) / ((ZTCG*ZFACX1I))
            ZPR02 = (ZRHO*ZICE0*YRECLDP%RCL_CONST1I) / ((ZTCG*ZFACX1I))

!!            ZTERM1 = ((ZVPLIQ - ZVPICE)*ZTP1(JL,JK,JBLK)**2.0_JPRB*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2I*ZFACX1I) /  &
            ZTERM1 = ((ZVPLIQ - ZVPICE)*ZTP1(JL,JK,JBLK)**2.0_JPRB*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2I*ZFACX1I) /  &
            & ((ZRHO*ZAPLUSB*ZVPICE))
...
!!            & (YRECLDP%RCL_CONST3I*ZCORRFAC**0.5_JPRB*ZRHO**0.5_JPRB*ZPR02**YRECLDP%RCL_CONST5I) / ZCORRFAC2**0.5_JPRB
            ...
            ZDEPOS = MAX(ZA(JL,JK,JBLK)*ZTERM1*ZTERM2*PTSPHY, 0.0_JPRB)
...
!!            IF (JK > YRECLDP%NCLDTOP) THEN
            IF (JK > YRECLDP%NCLDTOP) THEN
              ZFALLSRCE(JM) = ZPFPLSX(JL,JK, JM,JBLK)*ZDTGDP
...
!!            IF (YRECLDP%LAERICESED .and. JM == NCLDQI) THEN
        ...
        IF (ZQPRETOT > ZEPSEC) THEN
          ZCOVPTOT = 1.0_JPRB - ((1.0_JPRB - ZCOVPTOT)*(1.0_JPRB - MAX(ZA(JL,JK,JBLK), ZA(JL,JK - 1,JBLK)))) / (1.0_JPRB - MIN(ZA(JL,JK  &
          & - 1,JBLK), 1.0_JPRB - 1.E-06_JPRB))
!!          ZCOVPTOT = MAX(ZCOVPTOT, YRECLDP%RCOVPMIN)
          ZCOVPTOT = MAX(ZCOVPTOT, YRECLDP%RCOVPMIN)
          ZCOVPCLR = MAX(0.0_JPRB, ZCOVPTOT - ZA(JL,JK,JBLK))            ! clear sky proportion
        ...
        IF (ZTP1(JL,JK,JBLK) <= RTT) THEN
          !-----------------------------------------------------
          !     Snow Autoconversion rate follow Lin et al. 1983
          !-----------------------------------------------------
          IF (ZICECLD > ZEPSEC) THEN

!!            ZZCO = PTSPHY*YRECLDP%RSNOWLIN1*EXP(YRECLDP%RSNOWLIN2*(ZTP1(JL,JK,JBLK) - RTT))
            ZZCO = PTSPHY*YRECLDP%RSNOWLIN1*EXP(YRECLDP%RSNOWLIN2*(ZTP1(JL,JK,JBLK) - RTT))
...
!!                ZLCRIT = YRECLDP%RCLCRIT_SEA                  ! ocean
            ...
            !------------------------------------------------------------------
            ZPRECIP = (ZPFPLSX(JL,JK, NCLDQS,JBLK) + ZPFPLSX(JL,JK, NCLDQR,JBLK)) / MAX(ZEPSEC, ZCOVPTOT)
...
!!              ZRAINAUT = 1.5_JPRB*ZA(JL,JK,JBLK)*PTSPHY*YRECLDP%RCL_KKAAU*ZLIQCLD**YRECLDP%RCL_KKBAUQ*ZCONST**YRECLDP%RCL_KKBAUN
              ZRAINAUT = 1.5_JPRB*ZA(JL,JK,JBLK)*PTSPHY*YRECLDP%RCL_KKAAU*ZLIQCLD**YRECLDP%RCL_KKBAUQ*ZCONST**YRECLDP%RCL_KKBAUN

              ZRAINAUT = MIN(ZRAINAUT, ZQXFG(NCLDQL))
              IF (ZRAINAUT < ZEPSEC)               ZRAINAUT = 0.0_JPRB

!!              ZRAINACC = 2.0_JPRB*ZA(JL,JK,JBLK)*PTSPHY*YRECLDP%RCL_KKAAC*(ZLIQCLD*ZRAINCLD)**YRECLDP%RCL_KKBAC
              ZRAINACC = 2.0_JPRB*ZA(JL,JK,JBLK)*PTSPHY*YRECLDP%RCL_KKAAC*(ZLIQCLD*ZRAINCLD)**YRECLDP%RCL_KKBAC
        ...
        IF (IWARMRAIN > 1) THEN
          ...
          IF (ZTP1(JL,JK,JBLK) <= RTT .and. ZLIQCLD > ZEPSEC) THEN
...
!!              & 0.3_JPRB*ZCOVPTOT*PTSPHY*YRECLDP%RCL_CONST7S*ZFALLCORR*(ZRHO*ZSNOWCLD*YRECLDP%RCL_CONST1S)**YRECLDP%RCL_CONST8S
        ...
        IF (ZICETOT > ZEPSEC .and. ZTP1(JL,JK,JBLK) > RTT) THEN

          ! Calculate subsaturation
          ZSUBSAT = MAX(ZQSICE(JL,JK,JBLK) - ZQX(JL,JK, NCLDQV,JBLK), 0.0_JPRB)
          ...
          ! due to evaporation.
          ZTDMTW0 = ZTP1(JL,JK,JBLK) - RTT - ZSUBSAT*(ZTW1 + ZTW2*(PAP(JL, JK, JBLK) - ZTW3) - ZTW4*(ZTP1(JL,JK,JBLK) - ZTW5))
          ! Not implicit yet...
...
!!          ZCONS1 = ABS((PTSPHY*(1.0_JPRB + 0.5_JPRB*ZTDMTW0)) / YRECLDP%RTAUMEL)
        ...
        ! If rain present
        IF (ZQX(JL,JK, NCLDQR,JBLK) > ZEPSEC) THEN

          IF (ZTP1(JL,JK,JBLK) <= RTT .and. ZTP1(JL,JK - 1,JBLK) > RTT) THEN
            ...
            ZQPRETOT = MAX(ZQX(JL,JK, NCLDQS,JBLK) + ZQX(JL,JK, NCLDQR,JBLK), ZEPSEC)
            PRAINFRAC_TOPRFZ(JL,JBLK) = ZQX(JL,JK, NCLDQR,JBLK) / ZQPRETOT
...
!!              ZLAMBDA = (YRECLDP%RCL_FAC1 / ((ZRHO*ZQX(JK, NCLDQR))))**YRECLDP%RCL_FAC2
              ZLAMBDA = (YRECLDP%RCL_FAC1 / ((ZRHO*ZQX(JL,JK, NCLDQR,JBLK))))**YRECLDP%RCL_FAC2

              ! Calculate freezing rate based on Bigg(1953) and Wisner(1972)
!!              ZTEMP = YRECLDP%RCL_FZRAB*(ZTP1(JL,JK,JBLK) - RTT)
              ZTEMP = YRECLDP%RCL_FZRAB*(ZTP1(JL,JK,JBLK) - RTT)
...
!!              ZCONS1 = ABS((PTSPHY*(1.0_JPRB + 0.5_JPRB*(RTT - ZTP1(JL,JK,JBLK)))) / YRECLDP%RTAUMEL)
              ZCONS1 = ABS((PTSPHY*(1.0_JPRB + 0.5_JPRB*(RTT - ZTP1(JL,JK,JBLK)))) / YRECLDP%RTAUMEL)
              ZFRZMAX = MAX((RTT - ZTP1(JL,JK,JBLK))*ZCONS1*ZRLDCP, 0.0_JPRB)

            END IF

            IF (ZFRZMAX > ZEPSEC) THEN
              ZFRZ = MIN(ZQX(JL,JK, NCLDQR,JBLK), ZFRZMAX)
        ...
        !----------------------------------------------------------------------
        ! not implicit yet...
!!        ZFRZMAX = MAX((YRECLDP%RTHOMO - ZTP1(JL,JK,JBLK))*ZRLDCP, 0.0_JPRB)
        ZFRZMAX = MAX((YRECLDP%RTHOMO - ZTP1(JL,JK,JBLK))*ZRLDCP, 0.0_JPRB)
...
!!          ZZRH = YRECLDP%RPRECRHMAX + ((1.0_JPRB - YRECLDP%RPRECRHMAX)*ZCOVPMAX) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL,JK,JBLK))
          ZZRH = YRECLDP%RPRECRHMAX + ((1.0_JPRB - YRECLDP%RPRECRHMAX)*ZCOVPMAX) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL,JK,JBLK))
!!          ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
          ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)

          ZQE = (ZQX(JL,JK, NCLDQV,JBLK) - ZA(JL,JK,JBLK)*ZQSLIQ(JL,JK,JBLK)) / MAX(ZEPSEC, 1.0_JPRB - ZA(JL,JK,JBLK))
          !---------------------------------------------
          ! humidity in moistest ZCOVPCLR part of domain
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZQSLIQ(JL,JK,JBLK)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQR) > ZEPSEC .and. ZQE < ZZRH*ZQSLIQ(JL,JK,JBLK)
...
!!            ZBETA = RG*YRECLDP%RPECONS*0.5_JPRB*ZBETA1**0.5777_JPRB
            ...
            ZDPR = ((ZCOVPCLR*ZBETA*(ZQSLIQ(JL,JK,JBLK) - ZQE)) / ZDENOM)*ZDP*ZRG_R
            ZDPEVAP = ZDPR*ZDTGDP
            ...
            !-------------------------------------------------------------
!!            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, ((ZCOVPTOT - ZA(JL,JK,JBLK))*ZEVAP) / ZQXFG(NCLDQR)))
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, ((ZCOVPTOT - ZA(JL,JK,JBLK))*ZEVAP) / ZQXFG(NCLDQR)))
...
!!          ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
          ...
          ZQE = MAX(0.0_JPRB, MIN(ZQX(JL,JK, NCLDQV,JBLK), ZQSLIQ(JL,JK,JBLK)))
...
!!            ZFALLCORR = (YRECLDP%RDENSREF / ZRHO)**0.4
            ...
            ZESATLIQ = (RV / RD)*FOEELIQ(ZTP1(JL,JK,JBLK))

            ! Slope of particle size distribution
!!            ZLAMBDA = (YRECLDP%RCL_FAC1 / ((ZRHO*ZPRECLR)))**YRECLDP%RCL_FAC2              ! ZPRECLR=kg/kg
            ZLAMBDA = (YRECLDP%RCL_FAC1 / ((ZRHO*ZPRECLR)))**YRECLDP%RCL_FAC2              ! ZPRECLR=kg/kg

!!            ZEVAP_DENOM = YRECLDP%RCL_CDENOM1*ZESATLIQ - YRECLDP%RCL_CDENOM2*ZTP1(JL,JK,JBLK)*ZESATLIQ +  &
            ZEVAP_DENOM = YRECLDP%RCL_CDENOM1*ZESATLIQ - YRECLDP%RCL_CDENOM2*ZTP1(JL,JK,JBLK)*ZESATLIQ +  &
!!            & YRECLDP%RCL_CDENOM3*ZTP1(JL,JK,JBLK)**3._JPRB*PAP(JL, JK, JBLK)
            & YRECLDP%RCL_CDENOM3*ZTP1(JL,JK,JBLK)**3._JPRB*PAP(JL, JK, JBLK)

            ! Temperature dependent conductivity
            ZCORR2 = ((ZTP1(JL,JK,JBLK) / 273._JPRB)**1.5_JPRB*393._JPRB) / (ZTP1(JL,JK,JBLK) + 120._JPRB)
!!            ZKA = YRECLDP%RCL_KA273*ZCORR2
            ZKA = YRECLDP%RCL_KA273*ZCORR2

            ZSUBSAT = MAX(ZZRH*ZQSLIQ(JL,JK,JBLK) - ZQE, 0.0_JPRB)

!!            ZBETA = (0.5_JPRB / ZQSLIQ(JL,JK,JBLK))*ZTP1(JL,JK,JBLK)**2._JPRB*ZESATLIQ*YRECLDP%RCL_CONST1R*(ZCORR2 /  &
            ZBETA = (0.5_JPRB / ZQSLIQ(JL,JK,JBLK))*ZTP1(JL,JK,JBLK)**2._JPRB*ZESATLIQ*YRECLDP%RCL_CONST1R*(ZCORR2 /  &
...
!!          ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
          ...
          ! humidity in moistest ZCOVPCLR part of domain
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZQSICE(JL,JK,JBLK)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQS) > ZEPSEC .and. ZQE < ZZRH*ZQSICE(JL,JK,JBLK)
...
!!            ZBETA = RG*YRECLDP%RPECONS*ZBETA1**0.5777_JPRB
            ...
            ZDPR = ((ZCOVPCLR*ZBETA*(ZQSICE(JL,JK,JBLK) - ZQE)) / ZDENOM)*ZDP*ZRG_R
            ZDPEVAP = ZDPR*ZDTGDP
            ...
            ! 2-flux treatment, abandoned due to the new prognostic precip
            !-------------------------------------------------------------
!!            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, ((ZCOVPTOT - ZA(JL,JK,JBLK))*ZEVAP) / ZQXFG(NCLDQS)))
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, ((ZCOVPTOT - ZA(JL,JK,JBLK))*ZEVAP) / ZQXFG(NCLDQS)))
...
!!          ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
          ...
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZQSICE(JL,JK,JBLK)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQX(JL,JK, NCLDQS,JBLK) > ZEPSEC .and. ZQE < ZZRH*ZQSICE(JL,JK,JBLK)

          IF (LLO1) THEN

            ! Calculate local precipitation (kg/kg)
            ZPRECLR = ZQX(JL,JK, NCLDQS,JBLK) / ZCOVPTOT
            ZVPICE = (FOEEICE(ZTP1(JL,JK,JBLK))*RV) / RD
            ...
            ZAPLUSB =  &
!!            & YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL,JK,JBLK) + PAP(JL, JK, JBLK)*YRECLDP%RCL_APB3*ZTP1(JL,JK,JBLK)**3
            & YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZTP1(JL,JK,JBLK) + PAP(JL, JK, JBLK)*YRECLDP%RCL_APB3*ZTP1(JL,JK,JBLK)**3
            ZCORRFAC = (1.0 / ZRHO)**0.5
            ZCORRFAC2 = ((ZTP1(JL,JK,JBLK) / 273.0)**1.5)*(393.0 / (ZTP1(JL,JK,JBLK) + 120.0))

!!            ZPR02 = (ZRHO*ZPRECLR*YRECLDP%RCL_CONST1S) / ((ZTCG*ZFACX1S))
            ZPR02 = (ZRHO*ZPRECLR*YRECLDP%RCL_CONST1S) / ((ZTCG*ZFACX1S))

!!            ZTERM1 = ((ZQSICE(JL,JK,JBLK) - ZQE)*ZTP1(JL,JK,JBLK)**2*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2S*ZFACX1S) /  &
            ZTERM1 = ((ZQSICE(JL,JK,JBLK) - ZQE)*ZTP1(JL,JK,JBLK)**2*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2S*ZFACX1S) /  &
            & ((ZRHO*ZAPLUSB*ZQSICE(JL,JK,JBLK)))
...
!!            & (YRECLDP%RCL_CONST3S*ZCORRFAC**0.5*ZRHO**0.5*ZPR02**YRECLDP%RCL_CONST5S) / ZCORRFAC2**0.5
            ...
            ZEVAP = MIN(ZEVAP, ZQX(JL,JK, NCLDQS,JBLK))
            ...
            ! 2-flux treatment, abandoned due to the new prognostic precip
            !-------------------------------------------------------------
!!            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, ((ZCOVPTOT - ZA(JL,JK,JBLK))*ZEVAP) / ZQX(JK, NCLDQS)))
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, ((ZCOVPTOT - ZA(JL,JK,JBLK))*ZEVAP) / ZQX(JL,JK, NCLDQS,JBLK)))
...
!!            IF (ZQXFG(JM) < YRECLDP%RLMIN) THEN
        ...
        !---------------------------
        ZANEW = (ZA(JL,JK,JBLK) + ZSOLAC) / (1.0_JPRB + ZSOLAB)
        ZANEW = MIN(ZANEW, 1.0_JPRB)
!!        IF (ZANEW < YRECLDP%RAMIN)         ZANEW = 0.0_JPRB
        IF (ZANEW < YRECLDP%RAMIN)         ZANEW = 0.0_JPRB
        ZDA = ZANEW - ZAORIG(JL,JK,JBLK)
        !---------------------------------
...
!$claw nodep
        ...
        !---------------------------------------
        DO JM=1,NCLV
          ZMAX = MAX(ZQX(JL,JK, JM,JBLK), ZEPSEC)
        ...
        DO JM=1,NCLV
          ...
          !---------------------------
          ZMM = MAX(ZQX(JL,JK, JM,JBLK), ZEPSEC)
        ...
        DO JM=1,NCLV
          ...
          END DO
          ZQXN(JM) = ZQX(JL,JK, JM,JBLK) + ZEXPLICIT
        END DO
        ...
        DO JM=1,NCLV
          ZPFPLSX(JL,JK + 1, JM,JBLK) = ZFALLSINK(JM)*ZQXN(JM)*ZRDTGDP
        END DO

        ! Ensure precipitation fraction is zero if no precipitation
        ZQPRETOT = ZPFPLSX(JL,JK + 1, NCLDQS,JBLK) + ZPFPLSX(JL,JK + 1, NCLDQR,JBLK)
        IF (ZQPRETOT < ZEPSEC) THEN
          ZCOVPTOT = 0.0_JPRB
        END IF
        ...
        DO JM=1,NCLV - 1
          ...
          IF (IPHASE(JM) == 1) THEN
            TENDENCY_LOC(JL, JK, 1, JBLK) = TENDENCY_LOC(JL, JK, 1, JBLK) + RALVDCP*(ZQXN(JM) - ZQX(JL,JK, JM,JBLK) - ZFLUXQ(JM))*ZQTMST
          END IF

          IF (IPHASE(JM) == 2) THEN
            TENDENCY_LOC(JL, JK, 1, JBLK) = TENDENCY_LOC(JL, JK, 1, JBLK) + RALSDCP*(ZQXN(JM) - ZQX(JL,JK, JM,JBLK) - ZFLUXQ(JM))*ZQTMST
          END IF
          ...
          !----------------------------------------------------------------------
          TENDENCY_LOC(JL, JK, 3+JM, JBLK) = TENDENCY_LOC(JL, JK, 3+JM, JBLK) + (ZQXN(JM) - ZQX0(JL,JK, JM,JBLK))*ZQTMST
        ...
        ! 6.2 Humidity budget
        !----------------------
        TENDENCY_LOC(JL, JK, 3, JBLK) = TENDENCY_LOC(JL, JK, 3, JBLK) + (ZQXN(NCLDQV) - ZQX(JL,JK, NCLDQV,JBLK))*ZQTMST
...
!! !$acc loop seq
      DO JK=1,NLEV + 1
        PFPLSL(JL, JK, JBLK) = ZPFPLSX(JL,JK, NCLDQR,JBLK) + ZPFPLSX(JL,JK, NCLDQL,JBLK)
        PFPLSN(JL, JK, JBLK) = ZPFPLSX(JL,JK, NCLDQS,JBLK) + ZPFPLSX(JL,JK, NCLDQI,JBLK)
      END DO
...
!! !$acc loop seq
      DO JK=1,NLEV
        ...
        PFSQLF(JL, JK + 1, JBLK) = PFSQLF(JL, JK + 1, JBLK) + (ZQXN2D(JL,JK, NCLDQL,JBLK) - ZQX0(JL,JK, NCLDQL,JBLK) + PVFL(JL, JK, JBLK)*PTSPHY -  &
        & ZALFAW*PLUDE(JL, JK, JBLK))*ZGDPH_R
        ! liquid, negative numbers
        PFCQLNG(JL, JK + 1, JBLK) = PFCQLNG(JL, JK + 1, JBLK) + ZLNEG(JL,JK, NCLDQL,JBLK)*ZGDPH_R

        ! liquid, vertical diffusion
        PFSQLTUR(JL, JK + 1, JBLK) = PFSQLTUR(JL, JK + 1, JBLK) + PVFL(JL, JK, JBLK)*PTSPHY*ZGDPH_R

        ! Rain, LS scheme
        PFSQRF(JL, JK + 1, JBLK) = PFSQRF(JL, JK + 1, JBLK) + (ZQXN2D(JL,JK, NCLDQR,JBLK) - ZQX0(JL,JK, NCLDQR,JBLK))*ZGDPH_R
        ! rain, negative numbers
        PFCQRNG(JL, JK + 1, JBLK) = PFCQRNG(JL, JK + 1, JBLK) + ZLNEG(JL,JK, NCLDQR,JBLK)*ZGDPH_R

        ! Ice , LS scheme minus detrainment
        PFSQIF(JL, JK + 1, JBLK) = PFSQIF(JL, JK + 1, JBLK) + (ZQXN2D(JL,JK, NCLDQI,JBLK) - ZQX0(JL,JK, NCLDQI,JBLK) + PVFI(JL, JK, JBLK)*PTSPHY -  &
        & (1.0_JPRB - ZALFAW)*PLUDE(JL, JK, JBLK))*ZGDPH_R
        ! ice, negative numbers
        PFCQNNG(JL, JK + 1, JBLK) = PFCQNNG(JL, JK + 1, JBLK) + ZLNEG(JL,JK, NCLDQI,JBLK)*ZGDPH_R

        ! ice, vertical diffusion
        PFSQITUR(JL, JK + 1, JBLK) = PFSQITUR(JL, JK + 1, JBLK) + PVFI(JL, JK, JBLK)*PTSPHY*ZGDPH_R

        ! snow, LS scheme
        PFSQSF(JL, JK + 1, JBLK) = PFSQSF(JL, JK + 1, JBLK) + (ZQXN2D(JL,JK, NCLDQS,JBLK) - ZQX0(JL,JK, NCLDQS,JBLK))*ZGDPH_R
        ! snow, negative numbers
        PFCQSNG(JL, JK + 1, JBLK) = PFCQSNG(JL, JK + 1, JBLK) + ZLNEG(JL,JK, NCLDQS,JBLK)*ZGDPH_R
      END DO
...
END SUBROUTINE CLOUDSC_SCC_CUF_HOST_ALLOCATE
END MODULE CLOUDSC_GPU_SCC_CUF_HOST_ALLOCATE_MOD
